...
pipeline_dash = [{"$match": {"$and": [{"deleted": {"$ne": True}}, {"general.published": {"$ne": False}}]}},
    ...
    {"$project": {"dashboard": "$id", "metric_id": "$variable.k", "metric_del": "$variable.v.deleted",
    ...
    {"$project": {"dashboard": 1, "metric_id": 1, "metric_del": 1, "metric_pub": 1,
    ...
    {"$project": {"dashboard": 1, "metric_id": 1, "_id": 0}}]
